export type Testimonial = {
  ...
  highlight: string;
  /** Group tag shown on the note. Buyer-intent buckets: THINKING (still figuring
   *  out what to build), BUILDING (execution), STANDING (long-term / ownership). */
  group: "thinking" | "building" | "standing";
};

// Real client testimonials. Written from the reviewer's original words, tightened
// and voice-varied so no two open the same way and each carries one specific
// (a moment, a decision, a named artifact). Approved by the reviewers for use.
